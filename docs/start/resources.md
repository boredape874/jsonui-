---
title: 리소스
---


## 리소스 {#resources}





공식 문서, 커뮤니티, 툴을 함께 활용하면 학습 속도가 크게 올라갑니다.







### 초보자 추천 순서

1. Bedrock Wiki - JSON UI Intro
2. JSON UI Dumper (요소 찾기)
3. server_form.json (실전 구조)






### 바로 도움이 되는 도구

- JSON UI Dumper: 화면 요소 검색
- Bugrock Schemas: 자동완성/검증
- EasyUIBuilder: 빠른 시각화










### 공식 문서

- [Microsoft Learn - Creator Reference](https://learn.microsoft.com/en-us/minecraft/creator/reference/?view=minecraft-bedrock-stable)
- [Molang 소개](https://learn.microsoft.com/en-us/minecraft/creator/documents/molang/introduction)
- [Molang 문법 예제](https://learn.microsoft.com/en-us/minecraft/creator/reference/content/molangreference/examples/molangconcepts/molangintroduction?view=minecraft-bedrock-stable)
- [Bedrock Samples](https://github.com/Mojang/bedrock-samples)
- [server_form.json](https://github.com/Mojang/bedrock-samples/blob/main/resource_pack/ui/server_form.json)






### 커뮤니티 위키

- [Bedrock Wiki](https://wiki.bedrock.dev/)
- [JSON UI Intro](https://wiki.bedrock.dev/json-ui/json-ui-intro)
- [JSON UI Documentation](https://wiki.bedrock.dev/json-ui/json-ui-documentation)
- [Best Practices](https://wiki.bedrock.dev/json-ui/best-practices)
- [서버 폼 수정](https://wiki.bedrock.dev/json-ui/modifying-server-forms)






### 도구 & 시각화

- [JSON UI Dumper](https://theoristmc.github.io/JSON-UI-Dumper/)
- [JSON UI Editor](https://github.com/KalmeMarq/json-ui-editor)
- [EasyUIBuilder](https://github.com/GoldRush-developpement/EasyUIBuilder)
- [Bugrock JSON UI Schemas](https://github.com/KalmeMarq/Bugrock-JSON-UI-Schemas)
- [Minecraft Gradient Generator](https://minecraftcirclegenerator.app/minecraft-gradient-generator)






### 예제 코드 모음

- [Vanilla UI 파일 모음](https://github.com/bedrock-dot-dev/packs)
- [Vanilla Resource Pack](https://github.com/ZtechNetwork/MCBVanillaResourcePack)
- [JSON UI 예제 모음](https://github.com/LeGend077/json-ui-examples)
- [Super Deluxe UI](https://github.com/chiqors/Super-Deluxe-UI-MCBedrock)






### 라이브러리

- [StarLib 문서](https://pipangry.github.io/docs-starlib/)
- [StarLibV2 코드 모음](https://github.com/pipangry/StarLibV2)
- [JsonUI-Scripting](https://github.com/asakiyuki/jsonui-scripting)
- [bedrock-core/ui](https://github.com/bedrock-core/ui)






### ScriptAPI & 폼

- [ScriptAPI 커스텀 UI](https://lemon-slime.com/en/minecraft/create-custom-ui/)
- [서버 폼 수정](https://wiki.bedrock.dev/json-ui/modifying-server-forms)
- [manifest.json 가이드](https://learn.microsoft.com/en-us/minecraft/creator/reference/content/addonsreference/packmanifest?view=minecraft-bedrock-stable)
- [UUID Generator](https://www.uuidgenerator.net)






### 예제 UI 팩

- [Bedrock UI+](https://mcpedl.com/bedrock-ui-plus-b69-2/)
- [Another UI](https://mcpedl.com/another-ui/)
- [VDX Java UI](https://www.curseforge.com/minecraft-bedrock/addons/vdx-legacy-desktop-ui)
- [Deesse UI Pack](https://www.curseforge.com/minecraft-bedrock/texture-packs/deesse-ui-pack)
- [Bedrock Preview UI](https://mcpedl.com/bedrock-preview-ui/)






### VSCode & 스키마

- [Bedrock UI 확장](https://marketplace.visualstudio.com/items?itemName=Dingsel.bedrock-ui)
- [JSON Schema Validator](https://marketplace.visualstudio.com/items?itemName=tberman.json-schema-validator)
- [JSON Schema Diagnostics](https://marketplace.visualstudio.com/items?itemName=prosser.json-schema-2020-validation)
- [Schema Store](https://github.com/remcohaszing/vscode-schemastore)






### 커뮤니티 & 포럼

- [Bedrock 디스코드 모음](https://github.com/Bedrock-OSS/bedrock-wiki/blob/wiki/docs/discord.md)
- [BuiltByBit JSON UI](https://builtbybit.com/tags/json-ui/)
- [Minecraft Forum UI](https://www.minecraftforum.net/forums/minecraft-pocket-edition/mcpe-texture-packs/mcpe-texture-pack-help-requests/3146894-minecraft-bedrocks-ui-gui)






### 영상 & 튜토리얼

- [HUD 추가 가이드](https://wiki.bedrock.dev/json-ui/add-hud-elements)
- [Buttons & Toggles](https://wiki.bedrock.dev/json-ui/buttons-and-toggles)
- [Texture Atlas](https://wiki.bedrock.dev/concepts/texture-atlases)
- [Aseprite 애니메이션](https://wiki.bedrock.dev/json-ui/aseprite-animations)






### AI 학습 참고

- [Prompting Techniques](https://www.promptingguide.ai/techniques)
- [Prompt Engineering Guide](https://www.lakera.ai/blog/prompt-engineering-guide)
- [IBM Prompt Engineering](https://www.ibm.com/think/prompt-engineering)








처음엔 “초보자 추천 순서”의 3가지만 보면 충분합니다. 나머지는 필요할 때 찾아보세요.


      
