...
const renderSearchResults = (items, term, searchResults, searchCount, searchList) => {
  ...
  items.forEach((item) => {
    ...
    if (item.url) {
      const isExternal2 = /^https?:/i.test(item.url);
      link.href = isExternal2 ? item.url : `${base}${item.url}`;
    } else {
      link.href = `#${item.id}`;
    }
